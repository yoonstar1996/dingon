...
router.get("/list",async(req,res,next)=>{
    try{
        ...
        for (let i=0;i<data.length;i++){
            const query2 = `select * from subcomments inner join users on users.id=subcomments.userId where subcomments.id="${data[i].id}" order by subcomments.createdAt DESC`;
